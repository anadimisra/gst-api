...
public class SimpleIdentifiableResourceAssembler<T extends Identifiable<?>> extends SimpleResourceAssembler<T> {
	...
	/**
	 * The Spring MVC class for the {@link Identifiable} from which links will be
	 * built.
	 */
	private final Class<?> controllerClass;

	/**
	 * A {@link RelProvider} to look up names of links as options for resource
	 * paths.
	 */
	@Getter
	...
	/**
	 * Default a assembler based on Spring MVC controller, resource type, and
	...
	public SimpleIdentifiableResourceAssembler(Class<?> controllerClass, RelProvider relProvider) {
		...
		// SimpleIdentifiableResourceAssembler<User> -> User
		this.resourceType = GenericTypeResolver.resolveTypeArgument(this.getClass(),
				SimpleIdentifiableResourceAssembler.class);
	}
	...
	protected void addLinks(Resource<T> resource) {
		...
		resource.add(
				getCollectionLinkBuilder().withRel(this.relProvider.getCollectionResourceRelFor(this.resourceType)));
	}
	 ...
	 * links are put in the individual and collection resources.
	 *
	 * @return
	 */
